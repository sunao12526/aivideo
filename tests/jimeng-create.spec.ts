...
async function downloadImage(page: any, prompt: string, testIndex: number, imageIndex: number) {
  ...
  await page.locator(".button-group-gijUWa").first().waitFor({ state: "visible", timeout: 20000 });
  const downloadPromise = page.waitForEvent("download", { timeout: 20000 });
  await page.locator(".icon-kLbriy > svg").first().click({ timeout: 20000 });
  ...
  if (imageIndex < 3) { // 重新编辑操作只在前三张图片下载后执行
    ...
    await page.locator(".button-group-gijUWa").first().waitFor({ state: "hidden", timeout: 20000 });
  }
}

// 辅助函数：下载并保存普通图片
async function downloadImageLittle(page: any, prompt: string, testIndex: number) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const sanitizedPrompt = prompt.substring(0, 5).replace(/[^a-zA-Z0-9一-龥]/g, "_");
    const picFileName = `Little-${sanitizedPrompt}-${testIndex + 1}-${i + 1}.png`;
    const savePath = path.join(imagesPath, picFileName);
    console.log(`图片已保存至：${savePath}`);
...
test.describe.serial("Image Generation Tests", () => {
  for (const [index, prompt] of prompts.entries()) {
    test(`test-${index}`, async ({ page }) => {
      ...
      // await downloadImageLittle(page, prompt, index);
